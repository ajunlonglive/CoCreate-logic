...
const CoCreateLogic = {
			...
			inputs.forEach((input) => {
				...
				if (pass_value_id && pass_value_id == pass_value_to) {
					if (['INPUT', 'TEXTAREA', 'SELECT'].includes(input.tagName)) {
						...
						if (CoCreateFloatingLabel) CoCreateFloatingLabel.update(input)
					} else {
	...
	__setGetValueProcess: function(id, value) {
		...
		targets.forEach((target) => {
		  ...
		  if (CoCreateFloatingLabel) CoCreateFloatingLabel.update(target)
...
CoCreate.actions.registerEvent("passValueAction", CoCreateLogic.passProcessAction, CoCreateLogic, "passValueActionEnd");

// export default CoCreateLogic;
// export {CoCreateLogic}
// export default {CoCreateLogic, CoCreateAttributes}
